class PaintError(Exception):
  pass
class FormulaNotFound(PaintError):
...
class PaintMixer:
    ...
    def scale_formula(self, name, desired_batches):
        try:
            ...
            raise FormulaNotFound(name) from None
        ...
        for pigment,amount in pigments.items():
            new_amount=amount*(desired_batches/base_batches)
            scaled_pigments[pigment]=round(new_amount,2)
        return scaled_pigments
    def check_supplies(self, name, supplies):
        ...
        pigments=formula["pigments"]
        missing={}
        for pigment,required_amount in pigments.items():
           available=supplies.get(pigment,0)
           if available<required_amount:
              short=round(required_amount-available,2)
              missing[pigment]=short  
        if missing:
